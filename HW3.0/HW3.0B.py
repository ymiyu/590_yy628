...
def model(x,p):
	...
	if(model_type=="ANN"):
		px = extract_submatrices(p)
		out = np.transpose(x)
		for i in range(len(px)//2):
			out = np.matmul(px[i*2], out) + px[i*2+1]
			if i < len(px)//2-1:
...
def activation_fn(x):
	if activation=="TANH":
		out = (math.e**(x)-math.e**(-x))/(math.e**(x)+math.e**(-x))
		return out
	if activation == "RELU":
		out = np.where(x<0,0,x)
		return out
	if activation == "SIGMOID":
		out = math.e**(-x)/((1+math.e**(-x))**2)
		return out
...
def loss(p,index_2_use):
	...
	training_loss += GAMMA_L1*np.sum(np.abs(p))+GAMMA_L2*np.sum(p*p)
